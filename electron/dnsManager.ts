...
async function getActiveAdapter(): Promise<string> {
    ...
    for (const name of ['Wi-Fi', 'Ethernet', 'WiFi', 'Ethernet 2']) {
        try {
            ...
            if (stdout.includes('Connected')) return name;
        } catch {
            continue;
        }
    }
 ...
 */
async function getDNSFromNetsh(adapterName: string): Promise<{ dns: string[]; isDHCP: boolean }> {
    try {
        const { stdout } = await execAsync(
            `netsh interface ip show dns name="${adapterName}"`,
            { timeout: 10000, windowsHide: true }
        );
 ...
 */
async function getDNSFromConfig(adapterName: string): Promise<{ dns: string[]; isDHCP: boolean }> {
    try {
        const { stdout } = await execAsync(
 ...
 */
export async function applyDNS(primaryIP: string, secondaryIP: string): Promise<DNSResult> {
    try {
        ...
        await execAsync(
            `powershell -NoProfile -ExecutionPolicy Bypass -EncodedCommand ${outerBase64}`,
 ...
 */
export async function restoreDNS(): Promise<DNSResult> {
    try {
        const adapter = await getActiveAdapter();
 ...
 */
export async function getSystemDNS(): Promise<SystemDNSInfo> {
    try {
        const adapter = await getActiveAdapter();

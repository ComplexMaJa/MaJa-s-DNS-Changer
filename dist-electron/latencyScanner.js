 ...
 */
async function icmpTest(ip) {
    try {
        const { stdout } = await execAsync(`ping -n 1 -w ${ICMP_TIMEOUT} ${ip}`, { timeout: ICMP_TIMEOUT + 1000, windowsHide: true });
 ...
 */
function dnsQueryTest(dnsServer) {
...
function calculateJitter(tests) {
    const valid = tests.filter((t) => t !== TIMEOUT_VALUE);
    if (valid.length < 2)
        return 0;
...
function calculatePerformanceScore(stabilityScore, latencyScore) {
    return Math.round(stabilityScore * 0.6 + latencyScore * 0.4);
}
// ===================== Single Provider Benchmark =====================
async function benchmarkProvider(provider, testsPerMethod, onProgress) {
    const icmpResults = [];
    const dnsResults = [];
    const httpsResults = [];
    // Run ICMP tests
    ...
    const averageLatency = validAverages.length > 0
...
async function scanAllDNS(testsPerMethod, onProgress) {
    ...
    const processProvider = async (provider) => {
        ...
        try {
            const result = await benchmarkProvider(provider, testsPerMethod, (method, testIndex) => {
                onProgress({
                    providerName: provider.name,
                    primary: provider.primary,
                    secondary: provider.secondary,
